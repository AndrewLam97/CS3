...
void Land::destroyCircle(int x0, int y0, int radius)
{
    ...
    while( x < radius )
    {
        ...
        destroyColumn(x0-x,y0-y,y0+y);
        std::cout << "Debug Coords: " << x0 + x << " " << y0 - y << " " << y0 + y << std::endl; //#FIX
        ++x;
...
void Land::destroyColumn(int x,int top,int bottom)
{
    ...
    if(bottom > LandTop)
    {
        if(top > LandTop)
        {
            ...
            slManifest cav{top,bottom,50};
            for(auto i = slColList.begin() ;;)
            { 
...
{
    //Calculate weighted average of the solid pixels to get the norm
    float avgX = 0;
    float avgY = 0;
    int sq = 3;//size of the square
    for(int w = -sq; w <= sq; w++)
    {
      for(int h = -sq; h <= sq; h++)
      {
...
void Land::genHeightMap(Landtype Land_t) //Fix randomgen
{
    ...
    for(int i = 0; i < constants::windowWidth; i++)
    {
        ...
        {
            LandImg.setPixel(i, h, sf::Color::Green);
        }
    }
    LandTexture.loadFromImage(LandImg);
